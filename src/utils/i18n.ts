...
const productEnglish: Record<string, EnglishProductOverride> = {
  witnote: {
    ...
    tagline: "A local-first Markdown workspace: your notes stay in files you choose, and built-in AI runs on your device.",
    summary: "A Markdown workspace for Mac, iPhone, and iPad (Mac version available now; iPhone and iPad coming with 2.0.6). Notes stay in files you choose, with optional on-device AI. No account, no ads, no tracking.",
    audience: "Writers and researchers who want their Markdown files under their own control, with on-device AI when they want it.",
    platforms: ["macOS / iPhone / iPad"],
    status: ["Mac version on the App Store", "iPhone and iPad coming with 2.0.6", "Local-first", "No account"],
    privacyNote: "No account is required. User-selected folders, Markdown documents, and exports stay in the locations you choose, and the built-in on-device model does not upload documents. If you choose to use optional connected AI where it is available, text is sent to a service of your choice only when you send a request; on Mac, attached document content also requires a separate per-endpoint permission.",
    features: ["Markdown workspace for Mac, iPhone, and iPad", "Notes stay in files and folders you choose", "Built-in on-device AI for polishing, summaries, and outlines", "Open Markdown, Word, PDF, EPUB, and images; export to PDF, Word, HTML, text, and long images"],
    courseHooks: ["Native Swift macOS", "On-device AI", "Markdown knowledge workspaces", "App Store release compliance"],
  },
  agentlimb: {
  ...
  docktouchbar: {
    ...
    features: ["Persistent Dock on the Touch Bar, in the same order as the system Dock", "Tap to switch (jumps to another desktop when needed), double-tap to hide, long-press to quit, with a \"Closing…\" countdown at the right edge so your finger never hides it", "Recovers after sleep, screen unlock and Control Strip restarts", "A gear button at the right end hands the Touch Bar back to the system for a moment (brightness, volume) and returns automatically, so a fully dimmed screen never locks you out", "Rapid taps stay in order: the last tap wins, and it self-corrects if the system drops a switch or focus is stolen", "English and Simplified Chinese, menu bar settings and an About window"],
...
function buildEnglishDetail(product: Product, override: EnglishProductOverride) {
  ...
  if (product.slug === "witnote") {
    return {
      overview: [
        "One WitNote for Mac, iPhone, and iPad. The Mac version is on the App Store now; the iPhone and iPad versions are coming with 2.0.6.",
        "WitNote is free to download. WitNote Pro is a one-time purchase, shared across Mac, iPad, and iPhone (once the iPhone and iPad versions are out).",
        "No account, no ads, no tracking. Your notes stay in files and folders you choose.",
      ...
      proofPoints: [
        { label: "Platforms", value: "Mac / iPhone / iPad", note: "One app across Mac, iPhone, and iPad: the Mac version is on the App Store now, and iPhone and iPad are coming with 2.0.6. WitNote Pro is a one-time purchase shared across all three." },
        ...
        { label: "AI", value: "On-device AI", note: "A small built-in model runs on your device for polishing, summaries, grammar checks, titles, and outlines; other downloaded models are disclosed separately." },
      ...
      valueProps: [
        ...
        { title: "One app, three devices", body: "Use WitNote on Mac, iPhone, or iPad with the same workflow and the same one-time WitNote Pro purchase; the iPhone and iPad versions are coming with 2.0.6. The interface is available in 12 languages." },
      ...
      featureDetails: [
        { title: "Markdown workspace", body: "Keep notes, drafts, and HTML pages in files and folders you choose. Edit Markdown, preview the rendered result, and keep the source files available to other tools." },
        { title: "Local knowledge workflow", body: "Folders, pinning, and search by name, path, or full text turn your own directory into a lightweight knowledge base." },
        { title: "On-device AI", body: "A small built-in model runs on your device to polish a draft, pull out key points, check grammar, suggest titles, or outline an idea. It is optional, and speed and quality depend on your device." },
        { title: "Reading and conversion", body: "Open Markdown, text, HTML, Word, PDF, EPUB, and image files in one place. Convert and export to PDF, Word, HTML, plain text, or a phone-friendly long image; conversion and export are unlocked by WitNote Pro." },
        { title: "Release discipline", body: "Each release is checked as one chain: package signing, model notices, clean-start behavior, purchase restore, store metadata, and App Review evidence." },
      ],
      principles: [
        { title: "Keep the public promise reproducible", body: "The product page only describes the built-in model and verified workflows that the submitted app can actually reproduce on Mac, iPhone, and iPad." },
  ...
  if (product.slug === "agentlimb") {
    return {
      overview: [
        ...
        "b7 also announces official support for Chinese AI terminals: Tencent WorkBuddy, Alibaba Qwen Office, ByteDance TRAE Work, and Kimi Work. All four use the exact same onboarding protocol as Claude Code, Codex, Cursor, Trae, and Windsurf — copy the prompt from the side panel, paste it into the terminal, approve, and you are configured in 10 seconds. Combined with every Chromium browser (Chrome, Edge, Brave, Vivaldi), AgentLimb becomes a ready-to-use web-automation base for Chinese AI office scenarios."
      ],
      proofPoints: [
        { label: "Terminals", value: "5+ international / 4 Chinese", note: "Claude Code, Codex, Cursor, Trae, Windsurf — plus Tencent WorkBuddy, Alibaba Qwen Office, ByteDance TRAE Work, and Kimi Work." },
      ...
      valueProps: [
        { title: "Works with Chinese AI terminals out of the box", body: "WorkBuddy, Qwen Office, TRAE Work, and Kimi Work need zero special configuration — the same single onboarding prompt as international terminals. Chinese AI office suites get a properly adapted web-control capability for the first time." },
      ...
      featureDetails: [
        ...
        { title: "Official Chinese terminal support", body: "Tencent WorkBuddy, Alibaba Qwen Office, ByteDance TRAE Work, and Kimi Work are adapted via the standard protocol — no plugin-side changes needed." },
...
const englishVideoArticleOverrides: Record<string, EnglishVideoOverride> = {
  ...
  "video-workbuddy-lesson22-wechat-manager": {
    ...
    body: ["This lesson configures a local `wechat-manager` Skill so WorkBuddy can read, search, summarize, analyze, and export WeChat conversations through natural language.", "The course now provides one Windows + macOS auto-detect prompt. It checks the current system and usable local configuration before choosing a platform-specific read-only route; never mix the two platforms' low-level steps manually."],
    sections: [
      { heading: "Before you start | Third-party solution, account risk, and responsibility", paragraphs: ["This course uses a third-party open-source solution, not a Tencent or WeChat official feature. Reading, analyzing, or exporting local WeChat data or a running WeChat process may trigger account anomalies, restrictions, or WeChat account disablement. Platform rules, client implementations, and tools may change at any time.", "This material is for learning and teaching only. Test only on your own computer, your own test account, and the smallest possible data scope. The course and site accept no responsibility for account, data, business, privacy, or other loss resulting from its use. Never upload or share databases, keys, masterkeys, snapshots, passwords, or complete chat histories."] },
      { heading: "How to use this lesson", paragraphs: ["The bottom of this page has one Windows + macOS auto-detect Markdown download. Give it to WorkBuddy or another AI tool; it first checks the system, WeChat version, login state, actual Skills root, and usable local tools before choosing a platform-specific route. Read its read-only, privacy, batch-message, and stop-on-failure rules in full before acting.", "Use the order: check the environment, reuse a working existing configuration or complete the first installation check, create or read a local snapshot, verify a real session or group, then enable search, summaries, and exports. Do not repeat initialization, expose keys, or switch to unverified tools when a step fails."] },
      { heading: "Windows-specific notes", paragraphs: ["The Windows route has been validated on Windows 10 and WeChat 4.1.10.27: Config.Cipher scanning, HMAC validation, and local decrypted snapshots, without downgrading WeChat, using a debugger, DLL injection, or hooks. The unified prompt chooses this tested course route when it detects Windows.", "That result covers only the stated test environment, not every Windows or future WeChat version. In your own test environment, use the file's `doctor → groups → history → search` acceptance sequence. It is successful only after real messages and search results are returned."] },
