...
const englishVideoArticleOverrides: Record<string, EnglishVideoOverride> = {
  ...
  "video-workbuddy-lesson22-wechat-manager": {
    ...
    sections: [
      { heading: "Before you start | Third-party solution and risk notice", paragraphs: ["This course uses a third-party open-source solution, not a Tencent or WeChat official feature. Connecting to, reading, or analyzing local WeChat data may trigger account anomalies, restrictions, or bans. Use it only for learning and teaching after deciding whether the risk is acceptable. The course and site do not accept responsibility for account, data, business, or other losses caused by use.", "Never upload or share WeChat databases, key files, passwords, masterkeys, or complete chat histories. Test only on your own machine, your own test account, and the smallest possible data scope."] },
      { heading: "How to use this lesson", paragraphs: ["This page explains the boundaries and troubleshooting direction only. Download the complete Markdown configuration prompt at the bottom, then give it to WorkBuddy or another AI tool. Read its read-only, privacy, and large-message rules before validating each step for your operating system and WeChat version.", "Use the order: check the environment, confirm the WeChat version, initialize or perform a read test, verify recent sessions, then enable search, summaries, and exports. Do not repeatedly initialize or provide keys when a step fails."] },
      { heading: "Windows-specific notes", paragraphs: ["The course completed baseline testing on macOS. Windows has different paths, permissions, WeChat client structures, and installation methods, so do not copy the Mac flow directly. The verified legacy source documentation says its npm package provides a macOS arm64 binary only. On Windows, use `pip install wechat-cli` with Python 3.10 or later, or install from source.", "Keep WeChat signed in and open at least one conversation. Then run `wechat-cli init` in a sufficiently privileged Windows terminal and confirm the result with `wechat-cli sessions`. The original project does not state a required Windows WeChat client version, so neither “3.x only” nor “4.x never works” can be treated as a general conclusion. Validate in an isolated test environment; the original npm package is now marked deprecated."] },
      { heading: "Community troubleshooting lead (unofficial and unverified)", paragraphs: ["One user reported that, after the pip route failed on Windows, they used chatlogkeeper to obtain a masterkey. Because its key format differs from the CLI, they still needed to adapt the format and validate database decryption against the actual error. This is not part of the source project's verified installation route.", "This is a community report, not a reproducible promise or technical support. Never share masterkeys, key files, or real chat data. Stop if it cannot be verified in an isolated test environment."] },
    ],
  },
};
