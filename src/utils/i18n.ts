...
const englishVideoArticleOverrides: Record<string, EnglishVideoOverride> = {
  ...
  "video-workbuddy-lesson22-wechat-manager": {
    ...
    body: ["This lesson configures a local `wechat-manager` Skill so WorkBuddy can read, search, summarize, analyze, and export WeChat conversations through natural language.", "The course now has separate downloads: the Windows verified workflow and the macOS China-network optimized workflow. Use only the file that matches your computer; never mix installation steps across platforms."],
    sections: [
      ...
      { heading: "How to use this lesson", paragraphs: ["The bottom of this page has two separate Markdown downloads: the Windows verified prompt and the macOS China-network optimized prompt. Download the matching file, then give it to WorkBuddy or another AI tool. Read its read-only, privacy, batch-message, and stop-on-failure rules in full before acting.", "Use the order: check the environment, complete the installation check, initialize or create a snapshot, verify a session or group, then enable search, summaries, and exports. Do not repeat initialization, expose keys, or switch to unverified tools when a step fails."] },
      { heading: "Windows-specific notes", paragraphs: ["Windows users should not use the old npm/pip `wechat-cli` guidance and must not download the macOS file. The Windows download documents an actually validated Windows 10 and WeChat 4.1.10.27 read-only workflow: Config.Cipher scanning, HMAC validation, and local decrypted snapshots—without downgrading WeChat, using a debugger, DLL injection, or hooks.", "That result covers only the stated test environment, not every Windows or future WeChat version. In your own test environment, use the file's `doctor → groups → history` acceptance sequence. It is successful only after an active group returns real messages."] },
      { heading: "macOS and mainland-China network notes", paragraphs: ["The macOS download keeps the original `@canghe_ai/wechat-cli` structure and commands, but adds a one-time mainland-China npm mirror install with an official npm registry fallback. The mirror applies only to the install command and does not overwrite global npm settings. Verify `wechat-cli --help` before initialization.", "For network, certificate, DNS, or permission errors, follow the file's mirror/official-registry fallback and macOS Full Disk Access checks. Do not disable TLS verification, use long-term `sudo npm install -g` permission workarounds, or install identically named tools from untrusted mirrors, drives, or binaries."] },
    ],
  },
};
