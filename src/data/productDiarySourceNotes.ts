...
export const productDiarySourceNotes: Record<string, ProductDiarySourceNote> = {
  ...
  docktouchbar: {
    sourceAreas: ["项目开发记录", "私有接口可行性验证", "跨桌面切换与手势实验", "签名、公证与发布检查记录"],
    records: [
      "DockTouchBar 的开发记录从一次不满出发：现成的 Touch Bar Dock 工具功能太杂、用久了不稳定，于是把范围收窄到只做 Dock，并把稳定放在第一位。",
      "动手写产品之前先做了私有接口的可行性验证，发现旧方法名已被系统移除，也发现 isVisible 会说谎，这两条直接决定了后面的技术选型。",
      "跨桌面切换、双击隐藏（原计划是最小化，被台前调度挡住）、长按退出，每一个都有实测表格和被否掉的方案，而不是一次写对。",
      "发布链路记录了权限绑定签名、公证凭据、二进制里的本机路径这几个真实踩过的坑，以及为此重新编译并重新公证的过程。",
      "连点失效的排查记录了“先复现、分层找根因、从固定等待改成看结果、再加上持续纠正”的全过程，也记录了测试本身踩的坑：键鼠空闲判断、Touch Bar 触摸看不见、台前调度窗口干扰判据。",
      "1.5 记录了一次新机制的副作用：双击隐藏被“盯着结果并纠正”当成异常拉回来，修法是让隐藏也成为一次新点击并在隐藏后核对结果；这次没有拿到自动化数据，只有用户手动确认。",
      "1.6 记录了亮度调到 0 时的死循环和右端齿轮兜底：暂停与自动恢复的几种情况、被否掉的“调到 0 自动让位”（需要轮询亮度，会破坏空闲唤醒 0 次），以及哪些只能由用户在真机上验证。",
      "1.7 记录了齿轮改成小眼睛、临时隐藏时长可选，以及菜单的重新命名和分组（功能不变，只减少误解）。",
      "1.8 记录了长按提示从一条红色进度条到像素画四季的几次推翻重来，右侧按钮换成咖啡杯，以及哪些效果只能在真实窗口里验证、哪些只能由用户在真机上确认。",
    ],
    hardProblem: "最难的是让一个依赖私有接口的后台工具，在系统更新、睡眠唤醒和权限变化之后仍然稳定，并且在接口消失时安全地关闭功能。",
    reusableLesson: "小工具的价值来自克制和验证：先验证最大的不确定性，再写代码；发布前检查产物本身，而不只是源码。",
  },
  drowsebook: {
