...
export const expandedProductDiaries: ArticleSeed[] = [
  ...
  ...productSeries("docktouchbar", [
    ...
    },
    {
      ...
      sections: [
        ...
        },
        {
          ...
          paragraphs: [
            "点击一个图标时，程序按这样的顺序判断：如果 App 没在运行，或者已经被隐藏，就走普通的启动或显示；如果它在运行，先看当前桌面上有没有它的窗口，有就普通激活；当前桌面没有，再看别的桌面上有没有窗口（最小化的不算），有并且有辅助功能权限，才走“设为前台加 Raise”这条路；其他情况一律退回普通启动。"
          ],
        },
        {
          ...
          paragraphs: [
            ...
            "之后我给所有切换类测试加了一道门槛：读取系统的键鼠空闲时间，空闲超过 4 秒才开始；测完再确认期间没有输入，否则这次结果作废；每一步都切回原来的 App 和桌面。测试用的临时 App，也是专门编译的，不去碰正在使用的窗口。"
          ],
        },
        {
    ...
    },
    {
      ...
      title: "08 · 复盘：简洁、优雅、高效，落到了哪些数字和取舍上",
      category: "产品复盘",
      excerpt: "空闲 CPU 0.0%、内存 35 MB，这是量出来的。也记录这个项目的局限、许可证的取舍，以及和 AI 一起做的体会。",
      tags: ["复盘", "性能", "许可证"],
      order: 8,
      date: "2026-09-26",
      body: [
      ...
      sections: [
        {
          heading: "高效：量出来的数字",
          paragraphs: [
            "在 M1 的 MacBook Pro 上，用 Release 版本，Touch Bar 上显示着 Dock，键鼠空闲，连续 5 次采样，每次间隔 2 秒：CPU 占用全部是 0.0%，空闲唤醒 0 次，线程 3 个，物理内存约 35 MB，峰值 37 MB。App 体积 1.1 MB，其中包含 arm64 和 x86_64 两份，DMG 约 0.6 MB。代码约 1300 行 Swift，9 个文件，没有第三方依赖。",
            "同样重要的是没测的：点击时的响应延迟，设计上没有增加任何等待，但没有用仪器测过；有大量图标、长时间运行后的内存变化；以及和 Pock 等其他软件的对比。没测的东西，我就不在页面上写对比数字。"
          ],
        },
        {
          heading: "简洁和优雅：落到取舍上",
        ...
        },
        {
          heading: "和 AI 一起做的体会",
          paragraphs: [
            "有三点。第一，先验证，再写代码：让 AI 先去做探针和可行性验证，最大的不确定性消除之后，后面的路才短。第二，让 AI 检查它自己的产出：发布前的隐私扫描、二进制检查、文档里的数字和代码是否一致，都是让它去核对的，写文档时对照代码还发现了一个真实的边界 bug。第三，有些事必须由人来做：凭据的输入、许可证的取舍、“这个功能到底要不要加”的判断。AI 可以把选项和证据摆清楚，决定还是要人来下。",
