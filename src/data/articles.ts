...
const videoTutorialNoteSeeds: ArticleSeed[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        codeBlocks: [
          "西安未来智造数字科技有限公司是国家高新技术企业、科技型中小企业，2015 年起步，2018 年正式扎根西安，深耕工业智造领域。作为集 3D 打印、三维扫描、三维建模、CNC 精密加工、表面处理于一体的源头工厂，公司秉持“匠心铸精品、科技创价值”理念，为上万客户提供高精度工业模型制作、一站式个性化非标定制解决方案。\n\n我们根据这些信息生成一张 A4 宣传海报。顶部是公司名称，中间是公司介绍、配图、公司业务介绍，底部是电话。先不要着急生图，先沟通海报主题内容文字部分，也就是标题、介绍、业务介绍、联系方式等文案部分。然后再确定图片，最后组装成具有科技感的平面设计流程。",
  ...
  },
  {
    ...
    sections: [{ heading: "核心思路与步骤", paragraphs: ["先建立自己的资料库和文件夹：数据、规则、学生档案分别保存。这样可以减少 AI 幻觉，也让每个建议都能追溯。", "再明确三条路径：国内本科、港澳/中外合作、海外本科/预科；每条路径都按适配原因、主要风险、官方核验入口输出。", "最后让家长补齐缺失资料，而不是把模拟分数带当作真实录取线。没有官网、招生章程、当年计划或语言要求的结论，一律标记为“待核验”。"], codeBlocks: ["请读取当前工作区中的学校与专业数据、升学路径说明和学生需求档案。分别给出国内本科、港澳/中外合作、海外本科/预科的候选方向；每项写清适配原因、主要风险和需要核验的官方入口。不要使用“保证录取”“稳上”等表述；缺少当年官方资料时标记为“待核验”。"] }],
  },
  {
  ...
  },
  {
    ...
    sections: [{ heading: "核心步骤与提示词", paragraphs: ["先让采购助手把需求问完整：品名、规格、数量、预算、到货时间、供应商限制和验收标准。", "再生成采购清单 Excel，但不能把未经核验的价格、库存或供应商信息当成事实；最终下单、合同和付款仍需人工复核。"], codeBlocks: ["采购助手，帮我采购一批 LED 灯。先向我确认规格、数量、预算、到货时间、使用场景和供应商要求；再整理采购方案、待核验信息和风险项，不要直接下单。", "请把已确认的采购清单变成 Excel，包含品名、规格、数量、预算单价、预算总价、供应商、交期、核验状态和备注。未知信息留空并标记待确认。"] }],
  },
  {
    ...
    sections: [{ heading: "六步工作流", paragraphs: ["第一步搭工作区：创建 README、分析规则、工资字段说明、岗位字典、中间层和输出报告目录。", "第二步扫描原始资料；第三步建立字段映射；第四步清洗并建立标准工资数据库；第五步生成岗位、工资组成和部门人工成本分析；第六步用总额、随机抽样和交叉验证审计结果。", "遇到字段不确定、金额为空、重复员工或总额不一致，必须记录异常，不能自行猜测。"], codeBlocks: ["当前根目录只保留“原始工资表”文件夹。请不要立即分析工资；先建立适合长期维护的企业薪酬分析工作区：创建 README.md、分析规则.md、工资字段说明.md、岗位字典.xlsx、数据处理中间层和输出报告。不要修改原始工资表，完成后停止等待下一步。", "请扫描“原始工资表”，统计部门、文件数量、字段与差异，并生成“数据处理中间层/项目数据检查报告.md”。不要进行最终统计。", "请作为数据审计人员验证结果：比较原始工资表、标准工资数据库和年度薪酬分析报告的工资总额；随机抽查 10 名员工；检查部门工资总额是否等于岗位工资总额汇总。输出验证报告并列出具体异常。"] }],
  },
  {
  ...
  },
  {
    ...
    sections: [{ heading: "核心思路与步骤", paragraphs: ["文件夹就是业务阶段，移动文件就是更新进度，流程说明文件就是岗位规则，网页工作台就是可视化看板。先把岗位工作装进统一工作区，再考虑沉淀为固定的岗位专家和技能。", "案例工作区包含流程说明、网页工作台、01_新收材料、02_待制证、03_待受理办结、04_双公示待报送、05_待整理归档、06_已完成归档、07_模板与规则、08_周报与输出。", "课堂按六步演示：看原始问题、打开编号文件夹、查看业务记录、模拟流转、打开网页工作台、让 WorkBuddy 统计临期任务并生成周报草稿。正式系统中的受理、办结和提交仍需人工复核。"], codeBlocks: ["请读取当前工作区的文件夹结构、根目录《流程说明.md》以及各业务事项记录：\n1. 统计每个阶段的业务数量；\n2. 按截止日期找出最需要优先处理的事项；\n3. 检查业务是否缺少关键字段；\n4. 列出当前阶段、业务编号、单位名称、截止日期和下一步动作；\n5. 根据现有数据生成一份本周工作周报草稿；\n6. 不修改原始文件，不补全任何真实敏感信息。"] }],
  },
];
